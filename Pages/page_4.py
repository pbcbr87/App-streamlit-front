...
df_carteira['aporte_per'] = np.where(df_carteira['valor_mercado_brl'] == 0, 'Primeiro Aporte', df_carteira['aporte'] / df_carteira['valor_mercado_brl'])
...
st.write(df_carteira[['codigo_ativo', 'categoria','valor_mercado_br'l, 'aporte', 'aporte_per']].head(qt_ativo_aporte).sort_values(op_ordem[option], ascending=[False]).style.format(precision=2, thousands=".", decimal=",", subset=['aporte_per']))
